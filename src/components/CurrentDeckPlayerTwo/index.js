...
const CurrentDeckPlayerTwo = ({
  ...
  return (
    <>
      {p2.length % 2 !== 0 && p2.length ? (
        <div className={cls.card}>
          <div className={cls.card__top}>
            {
              p2[p2.length - 1]?.suits === 'clubs' ? '♧'
                : p2[p2.length - 1]?.suits === 'diamonds' ? '♢'
                  : p2[p2.length - 1]?.suits === 'hearts' ? '♡'
                    : p2[p2.length - 1]?.suits === 'spades' ? '♤'
                      : ''
            }
          </div>
          <div className={cls.card__icon}>{p2[p2.length - 1]?.name}</div>
      ...
      ) : p2.length ? <img className={cls.illusion} src={card} alt='card' /> : <div className={cls.card} />}
    </>
  )
}

export default CurrentDeckPlayerTwo